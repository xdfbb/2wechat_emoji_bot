...
public class CsdnList {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            //等待元素的加载。最大的时间30秒
            wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(CsdnBlog.URLS.substring(0, CsdnBlog.URLS.lastIndexOf("/")))));
        ...
        List<String> titles = jxDocument.selN(CsdnBlog.TITLES).stream().map(JXNode::asString).collect(Collectors.toList());
        List<String> blogUrls = jxDocument.selN(CsdnBlog.URLS).stream().map(JXNode::asString).collect(Collectors.toList());
        webDriver.close();
        webDriver.quit();
